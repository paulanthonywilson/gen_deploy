...
defmodule MixHelper do
  @moduledoc """
  Very much taken from https://github.com/phoenixframework/phoenix/blob/cbfc71c7fcd9c5b95ac5ed63719ac88a4927af85/installer/test/mix_helper.exs

  It simulates running in another app
  """
  ...
  def in_tmp_project(which, function) do
    conf_before = Application.get_env(:gen_deploy, :generators) || []
    path = Path.join([tmp_path(), random_string(10), to_string(which)])
    ...
    after
      File.rm_rf!(path)
      Application.put_env(:gen_deploy, :generators, conf_before)
    end
  end
  ...
  def with_generator_env(new_env, fun) do
    Application.put_env(:gen_deploy, :generators, new_env)
    try do
      fun.()
    after
      Application.delete_env(:gen_deploy, :generators)
    end
  end
